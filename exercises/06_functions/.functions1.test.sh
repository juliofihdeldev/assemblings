...
if [[ $code -eq 42 ]]; then
    echo "  Exit code correct: $code  (get_answer returned 42)"
    exit 0
else
    echo "  Expected exit code: 42  (returned by get_answer)"
    echo "  Got exit code:      $code"
    if [[ $code -gt 128 ]]; then
        echo "  The program crashed — likely missing the RET instruction."
    fi
    exit 1
fi
